...
engine.attach(piano); engine.attach(drums); engine.attach(ensemble); engine.attach(room); engine.attach(mastering)
let format = AVAudioFormat(standardFormatWithSampleRate: 44_100, channels: 2)!
room.loadFactoryPreset(.mediumRoom)
room.wetDryMix = 3
piano.masterGain = 0
drums.masterGain = -12
mastering.globalGain = 15
engine.connect(piano, to: ensemble, fromBus: 0, toBus: 0, format: format)
engine.connect(drums, to: ensemble, fromBus: 0, toBus: 1, format: format)
...
try piano.loadSoundBankInstrument(at: bankURL, program: 1, bankMSB: UInt8(kAUSampler_DefaultMelodicBankMSB), bankLSB: UInt8(kAUSampler_DefaultBankLSB))
...
if musicTracks.count >= 3 { musicTracks[2].destinationAudioUnit = drums }
